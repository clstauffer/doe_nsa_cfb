...
eff_bounds = np.array([0.,  2.7448,  4.1848,  7.3528, 13.1128, 37.0168]) # linear relationship
...
# for sidx, slf in zip([9],[0.999]): # SLF BINS
    for tidx, twp in enumerate(twp_bounds): # TWP BINS
        for didx,(dmv,eff) in enumerate(zip(dmv_bounds,eff_bounds)): # DMV BINS
            ...
            if (~np.isnan(eff)) & (len(eff_bins[((eff-2.5) <= eff_bins) & ((eff+2.5)>=eff_bins)]) > 1):
                ...
                # RUN RRTMG ######################################
                outfile   = '../data/rrtmg_sw/rrtmg_sw_output_MONTH'+str(month).zfill(2)+'_CBOT'+str(cloud_bot).zfill(4)+'_CHGT'+str(cloud_hgt).zfill(4)+'_SLF'+str(sidx).zfill(2)+'_TWP'+str(tidx).zfill(2)+'_DMV'+str(didx).zfill(2)+'_v4.1_APR2025.nc'
