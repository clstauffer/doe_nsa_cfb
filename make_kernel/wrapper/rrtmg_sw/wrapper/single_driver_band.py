...
import numpy as np
# import functions
import rrtmg_clr_func_band as rrtmg

def rrtmg_lw_driver( atmprofile,gas ):
    nlev    = np.size(atmprofile.pmid_tmp)
    if nlev==33:
        nlev=32

    atm     = rrtmg.read_profile(nlev,atmprofile )
    ...
    atm.ch4     = ch4
    atm.o2      = o2
    atm.semiss  = atmprofile.semiss #emis=1
    atm.sza     = atmprofile.sza
    atm.month   = atmprofile.month

    wbroadl = rrtmg.broad(atm,nlev)
    rrtmg.rrtmg_tape5_writer_htr_lw_band(atm,wbroadl,nlev)
