...
def rrtmg_tape5_writer_htr_lw_band(atm,gas,cflag,wbroadl,nlev):
    ...
    rdef1    = namedtuple('record1','iaer,iatm,iscat,istrm,iout,imca,icld,idelm,icos \
        juldat,sza,isolvar,scon,solcyfrac,solvar1,iemis,ireflect,semiss')
    record1  = rdef1(0,0,1,0,98,0,cflag.icld,0,0,0,atm.sza,2,0,0,'0.162822 2046.2270',1,0,atm.semiss)
    ...
    # record 1.2
    fileID.write('%20d%30d%33d%2d%5d%4d%1d%4d%1d\n' % (record1.iaer, record1.iatm, \
        record1.iscat, record1.istrm, record1.iout,\
        record1.imca, record1.icld,record1.idelm,record1.icos))

    # record 1.2.1
    fileID.write('%15d%10.4f%5d%10d%10.5f%140s\n' % (record1.juldat,\
        record1.sza, record1.isolvar,record1.scon,record1.solcyfrac, record1.solvar1))

    # record 1.4
    fileID.write('%12d%3d%5.3f\n' % (record1.iemis, \
...
def rrtmg_lw_htr(atm,nlev):
    ...
    os.system('/home/cls/projects/ctb-itan/cls/RRTMG/RRTMG_SW/run_examples_std_atm/rrtmg_sw') # replace with full path to ./wrapper
...
def rrtmg_lw_output_read_band(nlev):
    ...
    for j in range(0,nlev):
        ...
        fluxbroad.fdn[j] = tmp2[5]#[3]
        fluxbroad.fnt[j] = tmp2[6]#[4]
        fluxbroad.htr[j] = tmp2[7]#[5]

    fluxband.fup_band = np.full([14,nlev],np.nan)
    fluxband.fdn_band = np.full([14,nlev],np.nan)
    fluxband.fnt_band = np.full([14,nlev],np.nan)
    fluxband.htr_band = np.full([14,nlev],np.nan)

    for k in range(1,15):
        fi = open('OUTPUT_RRTM','r')
        lines = fi.readlines()[(nlev+6)*k+5:(nlev+6)*k+5+nlev]
        for j in range(0,nlev):
            tmp1 = lines[j]
            tmp2 = tmp1.split()
            for idx in range(len(tmp2)):
            ...
            fluxband.fdn_band[k-1,j] = tmp2[5]#[3]
            fluxband.fnt_band[k-1,j] = tmp2[6]#[4]
            fluxband.htr_band[k-1,j] = tmp2[7]#[5]
